...
class WorldGovBonds_Greece:
    ...
    def __init__(self):
        ...
        try:
            file = pd.read_csv("greek_worldgov.csv")
            file = True
        except:
            file = None
        ...
        if file:
            ...
            if str(today) in file["Date"].values:
                ...
                final_product.to_csv('greek_worldgov.csv', mode='a', index=False, header=False)
                print("File was updated")
        else:
            final_product.to_csv('greek_worldgov.csv', mode='w', index=False, header=True)
            print("Greek Bond Yield .csv was created")
